...
# todo reference
def opw(X: torch.Tensor, Y: torch.Tensor, lamda1=50, lamda2=0.1, delta=1, max_iter=20, verbose=0):
    """
    ...
    # fill p tensor
    i = torch.arange(1, n + 1, dtype=torch.float64).view(-1, 1)
    j = torch.arange(1, m + 1, dtype=torch.float64).view(1, -1)
    mid_para = torch.sqrt(torch.tensor(1 / (n ** 2) + 1 / (m ** 2)))
    d = (i / n - j / m).abs() / mid_para
    p = torch.exp(-d ** 2 / (2 * delta ** 2)) / (delta * torch.sqrt(2 * torch.tensor(np.pi)))

    #fill s tensor
    s = torch.zeros((n, m), dtype=torch.float64)
    i = torch.arange(1, n + 1, dtype=torch.float64).view(-1, 1)
    j = torch.arange(1, m + 1, dtype=torch.float64).view(1, -1)
    s = lamda1 / ((i / n - j / m) ** 2 + 1)

    # get the pairwise distances between X and Y (square euclidean)
    d = pdist2_EucSq(X, Y)
    ...
    compt = 0
    u = torch.ones((n, 1), dtype=torch.float64)/n
